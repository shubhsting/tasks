...
// assigning values to the images/or the div functions containing weekdays
    ...
    for (let i = 0; i < optionsArray.length; i++) {
        ...
        // for playing audio when user clicks on the object name
...
// it updates the question once it is answered correct.it plays the answer audio and reinitiates the process for next question
...
// it is used to get data from the given API
const getMethod = (url) => {
...
// It validates the answer that has been entered or selected by the user
...
// it is the first method which initiates the whole program
function renderStart(){
    ...
    btnStart.onclick = ()=>{
        if (startBtnClickCount === 0){
            ...
            // it initiates the screen which contains video for reference of the user
            openNav();
        } else {
            renderInit();
